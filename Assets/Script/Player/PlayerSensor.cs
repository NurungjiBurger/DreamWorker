...
public class PlayerSensor : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (!GameObject.Find("GameController").GetComponent<GameController>().IsPause)
        {
            ...
            if (collision.CompareTag("Wealth"))
            {
                ...
                {
                    if (collision.name.Equals("골드(Clone)")) GetComponent<PlayerStatus>().AddHandMoney(10);
                    if (collision.name.Equals("골드주머니(Clone)")) GetComponent<PlayerStatus>().AddHandMoney(30);
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (!GameObject.Find("GameController").GetComponent<GameController>().IsPause)
        {
            ...
            {
                if (collision.collider.name.Equals("골드(Clone)")) GetComponent<PlayerStatus>().AddHandMoney(10);
                if (collision.collider.name.Equals("골드주머니(Clone)")) GetComponent<PlayerStatus>().AddHandMoney(30);
                //if (collision.collider.name.Equals("스캐럽(Clone)")) GetComponent<PlayerStatus>().AddHandMoney(10);
